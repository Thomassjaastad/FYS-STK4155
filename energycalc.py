...
valid_states = states[:1000]
valid_energies = energies[:1000]
states = np.delete(states, np.s_[:1000], axis=0)
energies = np.delete(energies, np.s_[:1000], axis=0)
...
#network.earlystopping(train_states, energy_targets, valid_states, valid_energies)
